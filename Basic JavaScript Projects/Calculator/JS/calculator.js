...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator key is pressed, we conver the current number displayed on the screen to a number and then store the result in Calculator.First_Operand if it doesn't already exist
    const Value_of_Input = parseFloat(Display_Value);
    //checks if an operator already exists and if Wait_Second)Operand is true, then updates the operator and exits from the function
    ...
    if (First_Operand == null) {
        ...
        //if operator exists, property lookip is performed for the operatir in the Perform_Calculation object and the function that matches the operator is executed
